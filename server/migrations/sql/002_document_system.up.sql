...
-- Consolidated document, workflow, signature, serial, transmission, template, search, and document-authoring schema.
...
create table `document_workflow_events` (`id` bigint unsigned not null auto_increment primary key, `uuid` char(36) not null, `document_id` bigint unsigned not null, `actor_assignment_id` bigint unsigned not null, `action` varchar(80) not null, `from_status` varchar(60), `to_status` varchar(60), `from_unit_id` bigint unsigned, `to_unit_id` bigint unsigned, `note` text, `return_reason` text, `payload` json, `created_at` timestamp not null default CURRENT_TIMESTAMP);
...
create table `signature_events` (`id` bigint unsigned not null auto_increment primary key, `uuid` char(36) not null, `document_id` bigint unsigned not null, `user_id` bigint unsigned not null, `assignment_id` bigint unsigned not null, `signature_asset_id` bigint unsigned not null, `pin_verification_event_id` bigint unsigned not null, `serial_assignment_id` bigint unsigned, `status` varchar(40) not null default 'completed', `render_page` int unsigned, `render_x` decimal(10, 2), `render_y` decimal(10, 2), `render_width` decimal(10, 2), `render_height` decimal(10, 2), `ip_address` varchar(80), `user_agent` text, `created_at` timestamp not null default CURRENT_TIMESTAMP);
...
create table `document_renders` (`id` bigint unsigned not null auto_increment primary key, `uuid` char(36) not null, `document_id` bigint unsigned not null, `transmission_id` bigint unsigned, `file_asset_id` bigint unsigned, `render_key` varchar(160) not null, `render_type` varchar(80) not null default 'html_definition', `visibility_policy` varchar(80) not null default 'show_all', `source_version_number` int unsigned, `status` varchar(60) not null default 'generated', `created_by_assignment_id` bigint unsigned, `render_definition` json, `metadata` json, `created_at` timestamp not null default CURRENT_TIMESTAMP, `updated_at` timestamp not null default CURRENT_TIMESTAMP);
...
create table `notifications` (`id` bigint unsigned not null auto_increment primary key, `uuid` char(36) not null, `recipient_user_id` bigint unsigned not null, `recipient_assignment_id` bigint unsigned, `document_id` bigint unsigned, `transmission_id` bigint unsigned, `notification_type` varchar(120) not null, `channel` varchar(60) not null default 'in_app', `title` varchar(180) not null, `body` text, `status` varchar(60) not null default 'queued', `read_at` timestamp null, `sent_at` timestamp null, `failed_at` timestamp null, `failure_reason` text, `payload` json, `created_at` timestamp not null default CURRENT_TIMESTAMP, `updated_at` timestamp not null default CURRENT_TIMESTAMP);
